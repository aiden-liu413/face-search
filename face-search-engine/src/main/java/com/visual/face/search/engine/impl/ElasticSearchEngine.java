...
import com.visual.face.search.engine.model.SearchStatus;
...
public class ElasticSearchEngine implements SearchEngine {
    ...
    public boolean createCollection(String collectionName, MapParam param) {
        try {
            ...
            properties.put(Constant.ColumnNameFaceVector, Map.of("type", "dense_vector", "dims", "512"));
    ...
    public SearchResponse search(String collectionName, float[][] features, String algorithm, int topK) {
        try {
            ...
            for(float[] feature : features){
                ...
                Script script = new Script(Script.DEFAULT_SCRIPT_TYPE, "painless", "cosineSimilarity(params.query_value, doc['face_vector'])+1.0", params);
    ...
    public float searchMinScoreBySampleId(String collectionName, String sampleId,float[] feature, String algorithm) {
        try {
            ...
            SearchRequest searchRequest = new SearchRequest(collectionName).source(searchSourceBuilder);
            //搜索请求
            org.elasticsearch.action.search.SearchResponse response = this.client.search(searchRequest, RequestOptions.DEFAULT);
